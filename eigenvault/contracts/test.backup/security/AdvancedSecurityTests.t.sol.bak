...
contract AdvancedSecurityTests is Test {
    ...
    function testPrivilegeEscalation() public {
        ...
        vm.deal(ATTACKER, MIN_STAKE);
        vm.prank(ATTACKER);
        avs.registerOperator{value: MIN_STAKE}("privilege_attacker");
        
        // Try to call owner-only functions as operator
        vm.prank(ATTACKER);
        ...
        vm.prank(ATTACKER);
        vm.expectRevert(); // Should fail - not owner  
        avs.slashOperator(OPERATOR1, 1 ether, "unauthorized slash");
    ...
    function testSlashingResistance() public {
        // Test resistance to slashing manipulation
        vm.deal(OPERATOR1, MIN_STAKE * 2);
        vm.prank(OPERATOR1);
        avs.registerOperator{value: MIN_STAKE * 2}("slash_target");
        ...
        uint256 stakeAfterAttack = avs.getOperatorStake(OPERATOR1);
        assertEq(stakeAfterAttack, initialStake);
        
        // Owner can slash legitimately
        vm.prank(OWNER);
        avs.slashOperator(OPERATOR1, 1 ether, "legitimate slash");
        
        uint256 stakeAfterSlash = avs.getOperatorStake(OPERATOR1);
        assertEq(stakeAfterSlash, initialStake - 1 ether);
    }
