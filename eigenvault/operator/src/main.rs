...
impl Operator {
    ...
    async fn handle_p2p_message(&self, message: networking::P2PMessage) -> Result<()> {
        ...
        match message {
            P2PMessage::OrderGossip { order_id, encrypted_data } => {
    ...
    async fn handle_order_match(&self, order_match: matching::OrderMatch) -> Result<()> {
        ...
        let proof = self.zk_prover.generate_matching_proof(&order_match).await?;
        
        // Submit proof to Ethereum
        self.ethereum_client.submit_matching_proof(&order_match, proof).await?;
    ...
    async fn decrypt_orders(&self, encrypted_orders: Vec<Vec<u8>>) -> Result<Vec<matching::Order>> {
